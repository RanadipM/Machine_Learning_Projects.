...
C = dict(
    bg='#060B18', panel='#0D1526', surface='#111827',
    border='#1A2A45', border2='#2A3F5F',
    text='#CBD8F0', muted='#8AAED4', dim='#1A2A45',
...
def base_layout(fig, title="", height=440, subtitle=""):
    """Unified chart theme — Inter font, deeper contrast, FinIQ v6 branding."""
    full_title = (
        f"<b style='font-family:Space Grotesk,Inter,sans-serif'>{title}</b>"
        + (f"<br><sup style='color:#5A7A9E;font-size:11px'>{subtitle}</sup>" if subtitle else "")
    )
    fig.update_layout(
        paper_bgcolor=C["bg"],
        plot_bgcolor =C["panel"],
        font=dict(family="Inter, 'Segoe UI', sans-serif", color=C["text"], size=12),
        title=dict(text=full_title, font=dict(size=15, color=C["text"],
                   family="Space Grotesk, Inter, sans-serif"), x=0.02),
        height=height,
        margin=dict(l=60, r=30, t=65, b=50),
        legend=dict(
            bgcolor="rgba(13,21,38,0.85)", bordercolor=C["border2"], borderwidth=1,
            font=dict(size=11, family="Inter, sans-serif"),
            orientation="h", yanchor="bottom", y=1.02, xanchor="right", x=1),
        hoverlabel=dict(
            bgcolor=C["surface"], bordercolor=C["border2"],
            font=dict(color=C["text"], size=12, family="Inter, sans-serif")),
        modebar=dict(bgcolor="rgba(0,0,0,0)", color=C["muted"], activecolor=C["blue"]),
    )
    fig.update_xaxes(
        gridcolor="rgba(42,63,95,0.4)", linecolor=C["border2"],
        tickfont=dict(color=C["muted"], size=11, family="Inter, sans-serif"),
...
def wm(fig):
    """FinIQ v6 watermark on charts."""
    fig.add_annotation(
        text="FinIQ v6", xref="paper", yref="paper",
        x=0.99, y=0.01, showarrow=False,
        font=dict(size=10, color="rgba(90,122,158,0.5)", family="Inter, sans-serif"),
        xanchor="right")
    return fig
...
def fetch_with_cache(query: str) -> dict:
    """Fetch financials with smart cache fallback + retry on rate limit."""
    last_err = None
    ...
    for attempt in range(2):
        try:
            ...
            last_err = fetch_err
            err_s = str(fetch_err).lower()
            if any(k in err_s for k in ["429","too many","rate","throttle"]) and attempt == 0:
                time.sleep(3)   # wait 3s and retry once
                continue
    ...
    if cached is None:
        for suffix in ["", ".NS", ".BO"]:
            ...
            if cached: break
    if cached:
        cached["_fetch_error"] = str(last_err)
        return cached
    raise last_err  # no cache — propagate
...
def on_portfolio(t1: str, t2: str, t3: str, t4: str = "", t5: str = ""):
    ...
    # Chart 1: Altman Z bar — use full names, handle missing Z gracefully
    names  = [s["name"][:20] for s in stocks]  # truncate at 20 chars
    zvals  = [s.get("Z") or 0 for s in stocks]
    z_display = [f"{z:.2f}" if s.get("Z") else "N/A" for s, z in zip(stocks, zvals)]
    colors_z = []
    for s in stocks:
        z = s.get("Z")
        if z is None:   colors_z.append("#5A7A9E")   # grey = no data
        elif z >= 3:    colors_z.append("#22C55E")   # green = safe
        elif z >= 1.81: colors_z.append("#F59E0B")   # amber = grey zone
        else:           colors_z.append("#EF4444")   # red = distress
    ...
    fig1.add_trace(go.Bar(
        ...
        textfont=dict(color="#E2EAF8", size=12),
        hovertemplate="<b>%{x}</b><br>Altman Z: %{text}<extra></extra>"))
    if blended_z:
    ...
    fig1.update_layout(
        xaxis=dict(tickangle=-30, tickfont=dict(size=11, color="#CBD8F0")),
        yaxis=dict(range=[0, max(max(zvals or [5]), 3.5) * 1.2]),
        margin=dict(t=60, b=80, l=40, r=40))
    base_layout(fig1, f"Portfolio Altman Z-Score ({len(stocks)} stocks)", 400)
    ...
    md = (
        ...
        f"*Chart colours: 🟢 Safe (Z≥3) · 🟡 Grey zone (1.81-2.99) · 🔴 Distress (<1.81) · ⚫ No data*\n\n"
...
def _kpi_row(ratios, risk):
    """Return KPI summary as markdown — safe for gr.Markdown component."""
    Z, zone  = risk.get("Z"), risk.get("zone","N/A")
    M, m_lbl = risk.get("M"), risk.get("m_lbl","N/A")
    z_icon = "🟢" if "Safe" in str(zone) else ("🔴" if "Distress" in str(zone) else "🟡")
    m_icon = "🟢" if "No Man" in str(m_lbl) else "🔴"

    nm  = ratios.get("Net Margin %");   roe  = ratios.get("ROE %")
    de  = ratios.get("Debt-to-Equity"); fcf  = ratios.get("FCF Margin %")
    gm  = ratios.get("Gross Margin %"); cr   = ratios.get("Current Ratio")
    ic  = ratios.get("Interest Coverage"); at = ratios.get("Asset Turnover")
    roa = ratios.get("ROA %");          cfq  = ratios.get("Cash Flow Quality")
    gr_ = ratios.get("Graham Number")
    ...
    lines = [
        ...
        "|--------|-------|--------|--------|-------|--------|",
        f"| **Net Margin** | `{_fmt(nm,'%')}` | {'🟢' if (nm or 0)>15 else '🟡' if (nm or 0)>5 else '🔴'} | **ROE** | `{_fmt(roe,'%')}` | {'🟢' if (roe or 0)>20 else '🟡' if (roe or 0)>10 else '🔴'} |",
        f"| **FCF Margin** | `{_fmt(fcf,'%')}` | {'🟢' if (fcf or 0)>10 else '🟡'} | **Gross Margin** | `{_fmt(gm,'%')}` | {'🟢' if (gm or 0)>40 else '🟡'} |",
        f"| **D/E Ratio** | `{_fmt(de,'x')}` | {'🟢' if (de or 1)<0.5 else '🟡' if (de or 1)<1.5 else '🔴'} | **Current Ratio** | `{_fmt(cr,'x')}` | {'🟢' if (cr or 0)>2 else '🟡' if (cr or 0)>1.2 else '🔴'} |",
        f"| **ROA** | `{_fmt(roa,'%')}` | {'🟢' if (roa or 0)>10 else '🟡'} | **CF Quality** | `{_fmt(cfq,'x')}` | {'🟢' if (cfq or 0)>1 else '🟡'} |",
        f"| **Altman Z** | `{_fmt(Z)}` | {z_icon} *{zone}* | **Beneish M** | `{_fmt(M)}` | {m_icon} *{m_lbl[:20]}* |",
    ]
    if gr_:
        lines.append(f"| **Graham Number** | `₹{gr_:.0f}` | 💡 Intrinsic value est. | | | |")

    return "\n".join(lines)


def _status_html(msg: str, kind: str = "success") -> str:
    """Styled status message — returned as HTML string for gr.Markdown."""
    icons = {"success": "✅", "warning": "⚠️", "error": "❌", "info": "ℹ️"}
    return f"{icons.get(kind, 'ℹ️')} {msg}"

def on_voice_transcribe(audio):
    """
    Transcribe recorded audio using Groq Whisper.
    gr.Audio returns a tuple (sample_rate, numpy_array) when type='numpy',
    or a filepath when type='filepath'. We use filepath for simplicity.
    """
    if audio is None:
        return gr.update(), "⚠️ No audio recorded — tap the mic icon, speak, then click Stop, then Transcribe."
    if not groq_client:
        return gr.update(), "⚠️ FinAIKey2 (Groq) not set — voice transcription unavailable."
    try:
        # audio is a filepath string
        audio_path = audio if isinstance(audio, str) else audio[0] if isinstance(audio, (list,tuple)) else str(audio)
        with open(audio_path, "rb") as f:
            transcription = groq_client.audio.transcriptions.create(
                model="whisper-large-v3-turbo",
                file=f,
                language="en",
                prompt="Indian stock names: TCS, Infosys, Reliance, HDFC Bank, Wipro, Zomato, Airtel, Maruti, ITC, Titan"
            )
        text = transcription.text.strip()
        # Remove filler words
        noise = {"um","uh","hmm","okay","ok","please","search","show","analyse","analyze","fetch","the"}
        cleaned = " ".join(w for w in text.split() if w.lower() not in noise).strip()
        if not cleaned:
            return gr.update(), "⚠️ Could not make out a stock name — try again and speak clearly."
        return gr.update(value=cleaned), f"✅ Heard: **\"{cleaned}\"** — now click **Fetch & Analyse**"
    except Exception as e:
        return gr.update(), f"❌ Transcription error: {str(e)[:150]}"


def _build_figs(df, ratios, risk, ticker, info):
    """Build all 14 chart figures in the order expected by _all_figs / _fetch_outs."""
    Z    = risk.get("Z");  zone  = risk.get("zone","N/A")
    M    = risk.get("M");  m_lbl = risk.get("m_lbl","N/A")
    zcol = "#22C55E" if "Safe" in str(zone) else ("#EF4444" if "Distress" in str(zone) else "#F59E0B")

    def _safe(fn, *args, **kwargs):
        try:    return fn(*args, **kwargs)
        except: return None

    fig_rev = _safe(viz_revenue,       df)
    fig_mar = _safe(viz_margins,       df)
    fig_eps = _safe(viz_eps_trend,     df, info)
    fig_wf  = _safe(viz_waterfall,     df)
    fig_bs  = _safe(viz_balance_sheet, df)
    fig_cf  = _safe(viz_cashflow,      df)
    fig_dp  = _safe(viz_dupont,        df)
    fig_dcf = _safe(viz_dcf_heatmap,   df)
    fig_rg  = _safe(viz_ratio_gauges,  ratios)
    fig_az  = _safe(viz_altman_gauge,  Z, zone, zcol)
    fig_bm  = _safe(viz_beneish,       df)
    fig_rf  = _safe(viz_red_flags, df, ratios, Z, zone, M, m_lbl)

    # Price history — needs live ticker
    try:    fig_ph = viz_price_history(ticker) if ticker and ticker != "UPLOAD" else None
    except: fig_ph = None

    # Sentiment dashboard
    try:
        scores = generate_sentiment_scores(df, ratios, Z, M)
        fig_sd = viz_sentiment_dashboard(scores)
    except: fig_sd = None

    return (fig_rev, fig_mar, fig_eps, fig_wf,
...
def on_fetch(query):
    ...
    if not query or not query.strip():
        return ("⚠️ Enter a stock name or ticker above.", "", "*Fetch a company to see key metrics.*", "", *blank, "", "", "*Fetch a company to see bias analysis.*")
    try:
        raw = fetch_with_cache(query.strip())
    except Exception as e:
        err = str(e).lower()
        if any(k in err for k in ["429", "too many", "rate limit", "throttle"]):
            msg = "⏳ **Yahoo Finance rate limit.** Wait 60 seconds and retry, or type the NSE ticker directly: `TCS.NS` · `RELIANCE.NS`"
        elif "403" in err or "host not in allowlist" in err or "forbidden" in err:
            msg = "🔒 **Network blocked.** Run locally or upload an Excel file. On HuggingFace: enable outbound networking."
        else:
            msg = f"❌ {str(e)[:200]}"
        return (msg, "", "*Fetch a company to see key metrics.*", "", *blank, "", "", "*Fetch a company to see bias analysis.*")
    ...
    if from_cache:
        ...
        if stale:
            status = (f"⚠️ **Cached data is over 24 hours old** (cached: {cached_at[:16].replace('T',' ')}). "
                      f"Yahoo Finance unavailable — showing stale data.")
        else:
    ...
    return (status, header_with_sebi, _kpi_row(ratios, risk), peer_md,
            *figs, fore_md, bias_md)

def on_excel(file):
    blank = [None]*14
    if file is None:
        return ("⚠️ No file uploaded.", "", "*Fetch a company to see key metrics.*", "", *blank, "", "")
    try:    df = load_excel_and_normalize(file.name)
    except Exception as e:
        return (f"❌ {e}", "", "*Fetch a company to see key metrics.*", "", *blank, "", "")
    ...
    return ("✅  Excel loaded  ·  Years: " + years_str,
            ...
            _kpi_row(ratios, risk), peer_md, *figs, fore_md, bias_md)
...
def on_reset():
    ...
    blank = [None]*14
    empty_kpi = "*Fetch a company to see key metrics.*"
    return (
        "*Enter a stock name above and click **Fetch & Analyse***",
        "", empty_kpi, "",
        *blank,
        "", "*Fetch a company to see bias analysis.*"
    )
...
def on_export():
    ...
    try:
        ...
        if path and os.path.exists(path):
            return path, f"✅  Excel dashboard saved: {os.path.basename(path)}"
...
def export_premium_report():
    """Generate a presentation-ready Excel report with cover page, charts, and analysis."""
    if _df is None: return None, "⚠️ Fetch data first."
    try:
        from openpyxl import Workbook
        ...
        path      = f"/tmp/FinIQ_v6_{ticker_s}_Report.xlsx"
        ...
        # ── palette ──────────────────────────────────────────────────────────
        NAVY  = "060B18"; PANEL = "0D1526"; SURFACE = "111827"
        BLUE  = "3B82F6"; TEAL  = "14B8A6"; GREEN   = "22C55E"
        AMBER = "F59E0B"; RED   = "EF4444"; PURPLE  = "A78BFA"
        WHITE = "F0F6FF"; MUTED = "5A7A9E"; TEXT    = "CBD8F0"

        def _fill(h): return PatternFill("solid", fgColor=h)
        def _font(h, sz=10, bold=False, italic=False, name="Calibri"):
            return Font(name=name, color=h, size=sz, bold=bold, italic=italic)
        def _aln(h="left", v="center", wrap=False):
            return Alignment(horizontal=h, vertical=v, wrap_text=wrap)
        def _border(color="2A3F5F"):
            s = Side(border_style="thin", color=color)
            return Border(left=s, right=s, top=s, bottom=s)

        ann    = _df.sort_values("Year").reset_index(drop=True)
        years  = ann["Year"].tolist(); n = len(years)
        def _col(c): return ann[c].tolist() if c in ann.columns else [None]*n
        rev    = _col("Revenue"); ni    = _col("Net_Income")
        gp     = _col("Gross_Profit"); ebitda = _col("EBITDA")
        ocf    = _col("Operating_Cash_Flow"); fcf = _col("Free_Cash_Flow")
        equity = _col("Equity"); t_debt = _col("Total_Debt")

        def _pct(a,b): return round(a/b*100,1) if (a and b) else None
        gm  = [_pct(gp[i], rev[i])     for i in range(n)]
        nm  = [_pct(ni[i], rev[i])     for i in range(n)]
        em  = [_pct(ebitda[i], rev[i]) for i in range(n)]
        yoy = [None]+[_pct(rev[i]-rev[i-1], rev[i-1]) for i in range(1,n)]

        Z      = _risk.get("Z"); zone   = _risk.get("zone","N/A")
        M      = _risk.get("M"); m_lbl  = _risk.get("m_lbl","N/A")
        graham = _ratios.get("Graham Number")
        nm_r   = _ratios.get("Net Margin %"); roe_r = _ratios.get("ROE %")
        de_r   = _ratios.get("Debt-to-Equity"); fcf_r = _ratios.get("FCF Margin %")
        gross_r= _ratios.get("Gross Margin %")
        ...
        for r in range(1, 45):
            ws.row_dimensions[r].height = 18
            for c in range(1,10): ws.cell(r,c).fill = _fill(NAVY)

        # Title block
        ws.row_dimensions[3].height = 50; ws.row_dimensions[4].height = 28
        ws.row_dimensions[5].height = 22
        ws.merge_cells("B3:H3")
        c=ws["B3"]; c.value = f"FinIQ v6  ·  Investment Analysis Report"
        c.fill=_fill(NAVY); c.font=_font(MUTED,11,italic=True); c.alignment=_aln("left","center")

        ws.merge_cells("B4:H4")
        c=ws["B4"]; c.value = company
        c.fill=_fill(NAVY); c.font=Font(name="Calibri",color=WHITE,size=28,bold=True)
        c.alignment=_aln("left","center")

        ws.merge_cells("B5:H5")
        c=ws["B5"]; c.value = f"Ticker: {_ticker}  ·  Currency: {currency}  ·  Generated: {_dt.now().strftime('%d %b %Y %H:%M')}"
        c.fill=_fill(NAVY); c.font=_font(MUTED,10); c.alignment=_aln("left","center")

        # Gradient divider (fake — solid color bar)
        ws.row_dimensions[6].height = 4
        for ci in range(2,9): ws.cell(6,ci).fill = _fill(BLUE)
        ...
        kpis = [
            ("B","Net Margin",   f"{nm_r:.1f}%"  if nm_r   else "N/A", GREEN if (nm_r or 0)>15 else AMBER),
            ("C","ROE",          f"{roe_r:.1f}%"  if roe_r  else "N/A", GREEN if (roe_r or 0)>20 else AMBER),
            ("D","FCF Margin",   f"{fcf_r:.1f}%"  if fcf_r  else "N/A", TEAL),
            ("E","Altman Z",     f"{Z:.2f}"        if Z      else "N/A", GREEN if "Safe" in str(zone) else RED),
            ("F","Beneish M",    f"{M:.3f}"        if M      else "N/A", GREEN if "No Man" in str(m_lbl) else RED),
            ("G","Graham No.",   f"{sym}{graham:.0f}" if graham else "N/A", AMBER),
            ("H","D/E Ratio",    f"{de_r:.2f}x"   if de_r   else "N/A", GREEN if (de_r or 1)<0.5 else AMBER),
        ]
        for col_l, label, val, color in kpis:
            ws.cell(8,ord(col_l)-64).fill = _fill(color)   # accent top
            ws.cell(8,ord(col_l)-64).value = ""
            c=ws.cell(9,ord(col_l)-64); c.value=val
            c.fill=_fill(SURFACE); c.font=Font(name="Calibri",color=WHITE,size=16,bold=True)
            c.alignment=_aln("center","center")
            c=ws.cell(10,ord(col_l)-64); c.value=label
            c.fill=_fill(SURFACE); c.font=_font(MUTED,9); c.alignment=_aln("center","center")

        # Executive Summary
        ws.row_dimensions[12].height = 22
        ws.merge_cells("B12:H12")
        c=ws["B12"]; c.value="📊  Executive Summary"
        c.fill=_fill(PANEL); c.font=Font(name="Calibri",color="60A5FA",size=13,bold=True)
        c.alignment=_aln("left","center")

        # Auto-generate summary text
        trend = "growing" if (rev[-1] or 0) > (rev[0] or 0) else "declining"
        if n >= 2 and rev[0] and rev[-1]:
            cagr = ((rev[-1]/rev[0])**(1/max(n-1,1))-1)*100
            cagr_s = f"{cagr:.1f}% CAGR"
        else: cagr_s = "—"

        z_status  = "financially healthy" if (Z or 0)>2.99 else ("in grey zone" if (Z or 0)>1.81 else "under financial stress")
        m_status  = "no earnings manipulation detected" if (M or -3)<-2.22 else "possible earnings manipulation flagged"
        rec_score = sum([bool(nm_r and nm_r>10), bool(de_r and de_r<1),
                         bool(Z and Z>2.99), bool(roe_r and roe_r>15), bool(fcf_r and fcf_r>8)])
        rec = "STRONG BUY" if rec_score>=5 else ("BUY" if rec_score>=4 else ("HOLD" if rec_score>=2 else "REVIEW"))

        summary_lines = [
            f"{company} shows {trend} revenue with {cagr_s} over {n} years (FY{years[0]}–FY{years[-1]}).",
            f"Net margin stands at {nm_r:.1f}% with ROE of {roe_r:.1f}% — {'strong' if (nm_r or 0)>15 else 'moderate'} profitability profile.",
            f"Altman Z-Score of {Z:.2f} indicates the company is {z_status}.",
            f"Beneish M-Score of {M:.3f} — {m_status}.",
            f"Graham Number of {sym}{graham:.0f} provides an intrinsic value benchmark." if graham else "",
            f"FinIQ Verdict: {rec}  (based on {rec_score}/5 positive fundamental signals)",
        ]
        for i, line in enumerate(summary_lines):
            if not line: continue
            r = 13 + i; ws.row_dimensions[r].height = 20
            ws.merge_cells(f"B{r}:H{r}")
            c=ws.cell(r,2); c.value=f"  {line}"
            c.fill=_fill(PANEL if i%2==0 else NAVY)
            c.font=_font(TEXT,10); c.alignment=_aln("left","center")

        # Verdict box
        verdict_colors = {"STRONG BUY":GREEN,"BUY":"22D3EE","HOLD":AMBER,"REVIEW":RED}
        vc = verdict_colors.get(rec, AMBER)
        ws.row_dimensions[20].height = 30
        ws.merge_cells("B20:H20")
        c=ws["B20"]; c.value=f"  FinIQ Verdict: {rec}"
        c.fill=_fill(vc); c.font=Font(name="Calibri",color="000000",size=14,bold=True)
        c.alignment=_aln("left","center")

        # Footer
        ws.row_dimensions[22].height = 16
        ws.merge_cells("B22:H22")
        c=ws["B22"]; c.value="FinIQ v6  ·  Altman (1968) · Beneish (1999) · Graham & Dodd (1934)  ·  Data: Yahoo Finance  ·  NOT investment advice"
        c.fill=_fill(PANEL); c.font=_font(MUTED,8,italic=True); c.alignment=_aln("center","center")

        # ══════════════════════════════════════════════════════════════════════
        # SHEET 2 — FINANCIAL DASHBOARD (charts)
        # ══════════════════════════════════════════════════════════════════════
        ws2 = wb.create_sheet("📊 Dashboard")
        ws2.sheet_view.showGridLines = False
        ws2.sheet_properties.tabColor = TEAL
        ws2.column_dimensions["A"].width = 2
        for ci in range(2, 20): ws2.column_dimensions[get_column_letter(ci)].width = 14

        for r in range(1, 60):
            ws2.row_dimensions[r].height = 18
            for c in range(1,20): ws2.cell(r,c).fill = _fill(NAVY)

        # Section headers
        def _shdr(ws_, row, col, text, color=BLUE, span=7):
            ec = get_column_letter(col+span)
            ws_.merge_cells(f"{get_column_letter(col)}{row}:{ec}{row}")
            c_=ws_.cell(row,col); c_.value=text
            c_.fill=_fill(PANEL); c_.font=Font(name="Calibri",color=color,size=11,bold=True)
            c_.alignment=_aln("left","center")
            ws_.row_dimensions[row].height=22

        _shdr(ws2, 1, 2, f"📊  {company} — Financial Dashboard  ({currency} Millions)", WHITE, 15)
        _shdr(ws2, 2, 2, f"FY{years[0]} to FY{years[-1]}  ·  Generated {_dt.now().strftime('%d %b %Y')}  ·  FinIQ v6", MUTED, 15)

        # Hidden data table for charts — rows 4(header) + 5..5+n-1 (data)
        DC = 2  # start col B
        ws2.cell(4,DC).value="Year"
        hdrs_=["Revenue","Net Income","Gross Profit","EBITDA",
               "Gross Margin%","Net Margin%","EBITDA Margin%",
               "Op CF","FCF","Total Debt","Equity"]
        for i,h in enumerate(hdrs_): ws2.cell(4,DC+1+i).value=h
        for i in range(n):
            r_=5+i
            vals_=[years[i],
                   rev[i] if rev[i] else 0,
                   ni[i]  if ni[i]  else 0,
                   gp[i]  if gp[i]  else 0,
                   ebitda[i] if ebitda[i] else 0,
                   (gm[i]/100) if gm[i] else 0,
                   (nm[i]/100) if nm[i] else 0,
                   (em[i]/100) if em[i] else 0,
                   ocf[i] if ocf[i] else 0,
                   fcf[i] if fcf[i] else 0,
                   t_debt[i] if t_debt[i] else 0,
                   equity[i] if equity[i] else 0]
            for j,v in enumerate(vals_): ws2.cell(r_,DC+j).value=v

        dr0,dr1 = 5, 5+n-1
        # Categories = Year column (col B), rows 5..dr1
        cats = Reference(ws2, min_col=DC, min_row=dr0, max_row=dr1)

        chart_row1 = dr1 + 3   # charts start 3 rows after data
        chart_row2 = chart_row1 + 17

        _shdr(ws2, dr1+1, 2, f"Revenue vs Net Income ({currency} M)", BLUE, 7)
        _shdr(ws2, dr1+1, 10, "Profit Margins (%)", GREEN, 7)

        # Chart 1 — Revenue & Net Income (cols DC+1=C Revenue, DC+2=D Net Income)
        c1 = BarChart()
        c1.type = "col"; c1.grouping = "clustered"
        c1.title = None; c1.style = 10; c1.width = 16; c1.height = 11
        # Add Revenue
        d1 = Reference(ws2, min_col=DC+1, min_row=4, max_row=dr1)
        c1.add_data(d1, titles_from_data=True)
        # Add Net Income
        d2 = Reference(ws2, min_col=DC+2, min_row=4, max_row=dr1)
        c1.add_data(d2, titles_from_data=True)
        c1.set_categories(cats)
        c1.shape = 4
        try:
            c1.series[0].graphicalProperties.solidFill = "3B82F6"
            c1.series[1].graphicalProperties.solidFill = "14B8A6"
        except: pass
        c1.legend.position = "b"
        anchor1 = f"B{chart_row1}"
        ws2.add_chart(c1, anchor1)

        # Chart 2 — Margins (line chart)
        c2 = LineChart()
        c2.title = None; c2.style = 10; c2.width = 16; c2.height = 11
        for col_off in [DC+5, DC+6, DC+7]:  # Gross, Net, EBITDA margins
            data_ref = Reference(ws2, min_col=col_off, min_row=4, max_row=dr1)
            c2.add_data(data_ref, titles_from_data=True)
        c2.set_categories(cats)
        try:
            colors_m = ["22C55E", "3B82F6", "A78BFA"]
            for idx, cl in enumerate(colors_m):
                c2.series[idx].graphicalProperties.line.solidFill = cl
                c2.series[idx].graphicalProperties.line.width = 25000
                c2.series[idx].smooth = True
        except: pass
        c2.legend.position = "b"
        anchor2 = f"J{chart_row1}"
        ws2.add_chart(c2, anchor2)

        _shdr(ws2, chart_row1+16, 2, f"Cash Flow ({currency} M)", TEAL, 7)
        _shdr(ws2, chart_row1+16, 10, f"Capital Structure ({currency} M)", PURPLE, 7)

        # Chart 3 — Cash Flow (Op CF + FCF)
        c3 = BarChart()
        c3.type = "col"; c3.grouping = "clustered"
        c3.title = None; c3.style = 10; c3.width = 16; c3.height = 11
        d3 = Reference(ws2, min_col=DC+8, min_row=4, max_row=dr1)
        d4 = Reference(ws2, min_col=DC+9, min_row=4, max_row=dr1)
        c3.add_data(d3, titles_from_data=True)
        c3.add_data(d4, titles_from_data=True)
        c3.set_categories(cats)
        try:
            c3.series[0].graphicalProperties.solidFill = "14B8A6"
            c3.series[1].graphicalProperties.solidFill = "F59E0B"
        except: pass
        c3.legend.position = "b"
        ws2.add_chart(c3, f"B{chart_row2}")

        # Chart 4 — Debt vs Equity (stacked)
        c4 = BarChart()
        c4.type = "col"; c4.grouping = "stacked"
        c4.title = None; c4.style = 10; c4.width = 16; c4.height = 11
        d5 = Reference(ws2, min_col=DC+10, min_row=4, max_row=dr1)
        d6 = Reference(ws2, min_col=DC+11, min_row=4, max_row=dr1)
        c4.add_data(d5, titles_from_data=True)
        c4.add_data(d6, titles_from_data=True)
        c4.set_categories(cats)
        try:
            c4.series[0].graphicalProperties.solidFill = "EF4444"
            c4.series[1].graphicalProperties.solidFill = "22C55E"
        except: pass
        c4.legend.position = "b"
        ws2.add_chart(c4, f"J{chart_row2}")

        # ══════════════════════════════════════════════════════════════════════
        # SHEET 3 — ANNUAL DATA TABLE
        # ══════════════════════════════════════════════════════════════════════
        ws3 = wb.create_sheet("📋 Annual Data")
        ws3.sheet_view.showGridLines = False
        ws3.sheet_properties.tabColor = "6366F1"
        ws3.column_dimensions["A"].width = 2
        ws3.column_dimensions["B"].width = 30
        for ci in range(3, 3+n+2): ws3.column_dimensions[get_column_letter(ci)].width = 16

        for r in range(1, 40):
            ws3.row_dimensions[r].height = 21
            for c in range(1,15): ws3.cell(r,c).fill = _fill(NAVY)

        ws3.row_dimensions[1].height=32
        ws3.merge_cells(f"B1:{get_column_letter(3+n)}1")
        c=ws3["B1"]; c.value=f"  {company} — Annual Financials  ({currency} Millions)"
        c.fill=_fill(PANEL); c.font=Font(name="Calibri",color=WHITE,size=14,bold=True)
        c.alignment=_aln("left","center")

        # Year headers
        for i,yr in enumerate(years):
            c=ws3.cell(2,3+i); c.value=str(yr)
            c.fill=_fill(SURFACE); c.font=Font(name="Calibri",color="60A5FA",size=11,bold=True)
            c.alignment=_aln("center","center")
        ws3.cell(2,3+n).value="YoY"; ws3.cell(2,3+n).fill=_fill(SURFACE)
        ws3.cell(2,3+n).font=Font(name="Calibri",color="60A5FA",size=11,bold=True)
        ws3.cell(2,3+n).alignment=_aln("center","center")
        ...
        sections=[
            ...
            ("Net Margin %",    nm,     True),
            ("EBITDA Margin %", em,     True),
            ("YoY Revenue %",   yoy,    True),
            ("── CASH FLOW", None, None),
            ("Operating CF",    ocf,    False),
            ("Free Cash Flow",  fcf,    False),
            ("── BALANCE", None, None),
            ("Total Debt",      t_debt, False),
            ("Equity",          equity, False),
        ]
        row_=3
        for label_,data_,is_pct in sections:
            ws3.row_dimensions[row_].height=21
            is_sec=label_.startswith("──")
            if is_sec:
                ws3.merge_cells(f"B{row_}:{get_column_letter(3+n)}{row_}")
                c=ws3.cell(row_,2); c.value=f"  {label_[3:]}"
                c.fill=_fill(SURFACE); c.font=Font(name="Calibri",color="60A5FA",size=10,bold=True)
                c.alignment=_aln("left","center"); row_+=1; continue
            bg=PANEL if row_%2==0 else NAVY
            c=ws3.cell(row_,2); c.value=f"  {label_}"
            c.fill=_fill(bg); c.font=_font(TEXT,10); c.alignment=_aln("left","center")
            # YoY for this row
            row_yoy = None
            if data_ and len(data_)>=2:
                try:
                    if not is_pct and data_[-1] and data_[-2]:
                        row_yoy = f"{'▲' if data_[-1]>=data_[-2] else '▼'} {abs((data_[-1]-data_[-2])/data_[-2]*100):.1f}%"
                except: pass
            for i,(v) in enumerate(data_):
                c=ws3.cell(row_,3+i); c.fill=_fill(bg)
                if v is None: c.value="—"; c.font=_font(MUTED,10)
                elif is_pct:
                    c.value=v/100 if v else 0; c.number_format="0.0%"
                    clr=GREEN if (v or 0)>=0 else RED
                    c.font=Font(name="Calibri",color=clr,size=10,bold=True)
                else:
                    c.value=v; c.number_format="#,##0"
                    c.font=_font(TEXT,10)
                c.alignment=_aln("center","center")
            # YoY col
            yoy_c=ws3.cell(row_,3+n)
            if row_yoy:
                yoy_c.value=row_yoy
                yoy_c.font=Font(name="Calibri",color=GREEN if "▲" in row_yoy else RED,size=10,bold=True)
            else: yoy_c.value="—"; yoy_c.font=_font(MUTED,10)
            yoy_c.fill=_fill(bg); yoy_c.alignment=_aln("center","center")
            row_+=1

        # ══════════════════════════════════════════════════════════════════════
        # SHEET 4 — RISK & RATIOS
        # ══════════════════════════════════════════════════════════════════════
        ws4=wb.create_sheet("⚠️ Risk & Ratios")
        ws4.sheet_view.showGridLines=False; ws4.sheet_properties.tabColor=RED
        ws4.column_dimensions["A"].width=2
        ws4.column_dimensions["B"].width=32; ws4.column_dimensions["C"].width=20
        ws4.column_dimensions["D"].width=2
        ws4.column_dimensions["E"].width=32; ws4.column_dimensions["F"].width=20
        for r in range(1,30):
            ws4.row_dimensions[r].height=22
            for c in range(1,8): ws4.cell(r,c).fill=_fill(NAVY)

        ws4.row_dimensions[1].height=32; ws4.merge_cells("B1:F1")
        c=ws4["B1"]; c.value=f"  {company} — Risk Assessment & Key Ratios"
        c.fill=_fill(PANEL); c.font=Font(name="Calibri",color=WHITE,size=14,bold=True); c.alignment=_aln("left","center")

        risk_rows=[
            ("Altman Z-Score",  f"{Z:.3f}" if Z else "N/A",   zone,    GREEN if "Safe" in str(zone) else (AMBER if "Grey" in str(zone) else RED)),
            ("Beneish M-Score", f"{M:.3f}" if M else "N/A",   m_lbl,   GREEN if "No Man" in str(m_lbl) else RED),
            ("Graham Number",   f"{sym}{graham:.0f}" if graham else "N/A", "Intrinsic value est.", AMBER),
            ("D/E Ratio",       f"{de_r:.3f}x" if de_r else "N/A", "Low leverage" if (de_r or 1)<0.5 else "Moderate", GREEN if (de_r or 1)<0.5 else AMBER),
            ("Current Ratio",   f"{_ratios.get('Current Ratio','N/A')}", "Liquidity", TEAL),
            ("Interest Coverage",f"{_ratios.get('Interest Coverage','N/A')}", "Debt service", TEAL),
        ]
        ws4.merge_cells("B2:C2"); c=ws4["B2"]; c.value="⚠️  Risk Models"
        c.fill=_fill(SURFACE); c.font=Font(name="Calibri",color="F59E0B",size=11,bold=True); c.alignment=_aln("left","center")
        for i,(lbl,val,note,color) in enumerate(risk_rows):
            r_=3+i; bg=SURFACE if i%2==0 else NAVY
            c=ws4.cell(r_,2); c.value=lbl; c.fill=_fill(bg); c.font=_font(TEXT,10,True); c.alignment=_aln("left","center")
            c=ws4.cell(r_,3); c.value=val; c.fill=_fill(bg); c.font=Font(name="Calibri",color=color,size=12,bold=True); c.alignment=_aln("center","center")

        ratio_rows=[
            ("Gross Margin %", f"{gross_r:.1f}%" if gross_r else "N/A", GREEN if (gross_r or 0)>40 else AMBER),
            ("Net Margin %",   f"{nm_r:.1f}%"    if nm_r    else "N/A", GREEN if (nm_r or 0)>15 else AMBER),
            ("ROE %",          f"{roe_r:.1f}%"   if roe_r   else "N/A", GREEN if (roe_r or 0)>20 else AMBER),
            ("ROA %",          f"{_ratios.get('ROA %','N/A')}", TEAL),
            ("FCF Margin %",   f"{fcf_r:.1f}%"   if fcf_r   else "N/A", GREEN if (fcf_r or 0)>10 else AMBER),
            ("Asset Turnover", f"{_ratios.get('Asset Turnover','N/A')}", TEAL),
        ]
        ws4.merge_cells("E2:F2"); c=ws4["E2"]; c.value="📐  Key Ratios"
        c.fill=_fill(SURFACE); c.font=Font(name="Calibri",color="60A5FA",size=11,bold=True); c.alignment=_aln("left","center")
        for i,(lbl,val,color) in enumerate(ratio_rows):
            r_=3+i; bg=SURFACE if i%2==0 else NAVY
            c=ws4.cell(r_,5); c.value=lbl; c.fill=_fill(bg); c.font=_font(TEXT,10,True); c.alignment=_aln("left","center")
            c=ws4.cell(r_,6); c.value=val; c.fill=_fill(bg); c.font=Font(name="Calibri",color=color,size=12,bold=True); c.alignment=_aln("center","center")

        # AI insights if available
        if _insights:
            ws4.row_dimensions[12].height=22; ws4.merge_cells("B12:F12")
            c=ws4["B12"]; c.value="🤖  AI Analysis Summary"
            c.fill=_fill(SURFACE); c.font=Font(name="Calibri",color="60A5FA",size=11,bold=True); c.alignment=_aln("left","center")
            clean=_insights.replace("**","").replace("##","").replace("#","").replace("*","")
            for i,line in enumerate(clean.split("\n")[:12]):
                if not line.strip(): continue
                r_=13+i; ws4.row_dimensions[r_].height=18; ws4.merge_cells(f"B{r_}:F{r_}")
                c=ws4.cell(r_,2); c.value=f"  {line.strip()[:120]}"
                c.fill=_fill(PANEL if i%2==0 else NAVY); c.font=_font(TEXT,9); c.alignment=_aln("left","center",wrap=True)

        # Footer
        ws4.row_dimensions[28].height=16; ws4.merge_cells("B28:F28")
        c=ws4["B28"]; c.value="FinIQ v6  ·  Altman (1968) · Beneish (1999) · Graham & Dodd (1934)  ·  NOT investment advice"
        c.fill=_fill(PANEL); c.font=_font(MUTED,8,italic=True); c.alignment=_aln("center","center")

        wb.save(path)
        return path, f"✅ Premium report saved: FinIQ_v6_{ticker_s}_Report.xlsx"
    except Exception as e:
        import traceback; traceback.print_exc()
        return None, f"❌ Report error: {e}"


def on_premium_export():
    path, msg = export_premium_report()
    return path, msg
...
def on_pdf_export():
    ...
    try:
        ...
        if _insights:
            ...
            # Clean markdown
            clean = _insights.replace("**","").replace("##","").replace("#","").replace("*","")
            # Check if content is non-ASCII (Hindi/Bengali etc.) — PDF can't render without special fonts
            non_ascii_ratio = sum(1 for c in clean if ord(c) > 127) / max(len(clean), 1)
            if non_ascii_ratio > 0.3:
                # Non-Latin script — show a note and the English rule-based analysis instead
                story.append(Paragraph(
                    "Note: AI analysis was generated in a non-Latin language. "
                    "Please regenerate in English for PDF export, or view in the app.",
                    ps("note", fontSize=9, textColor=colors.HexColor("#F59E0B"), italic=True)))
                # Fall back to rule-based insights
                rb = _rule_based_insights(_df, _ratios,
                                          _risk.get("Z"), _risk.get("zone","N/A"),
                                          _risk.get("M"), _risk.get("m_lbl","N/A"))
                rb_clean = rb.replace("**","").replace("##","").replace("#","").replace("*","")
                for line in rb_clean.split("\n"):
                    if line.strip():
...
h1 { font-family:'Space Grotesk',sans-serif !important; font-size:28px !important; font-weight:700 !important; color:#F0F6FF !important; letter-spacing:-0.02em !important; }
h2 { font-family:'Space Grotesk',sans-serif !important; font-size:20px !important; font-weight:600 !important; color:#E2EAF8 !important; border-bottom:1px solid rgba(77,159,255,0.15) !important; padding-bottom:6px !important; margin-bottom:12px !important; }
h3 { font-family:'Space Grotesk',sans-serif !important; font-size:16px !important; font-weight:600 !important; color:#60A5FA !important; }
h4 { font-family:'Inter',sans-serif !important; font-size:14px !important; font-weight:600 !important; color:#94B4D8 !important; }

/* Prose text — INTER not monospace */
.prose p, .prose li, .prose td, .prose th, .prose blockquote {
    font-family: 'Inter', sans-serif !important;
    font-size: 14px !important;
    line-height: 1.75 !important;
    color: #CBD8F0 !important;
}
.prose strong { color: #E2EAF8 !important; font-weight: 600 !important; }
.prose h2 { color: #60A5FA !important; }
.prose h3 { color: #94B4D8 !important; }
.prose code {
    font-family: 'JetBrains Mono', monospace !important;
    background: rgba(77,159,255,0.1) !important;
    border-radius: 4px !important; padding: 2px 6px !important;
    color: #7DD3FC !important; font-size: 12.5px !important;
}
/* Devanagari / Indian scripts */
.prose p, .markdown { font-family: 'Noto Sans Devanagari','Inter',sans-serif !important; }

/* Data tables — mono */
.dataframe td, .dataframe th, code, pre {
    font-family: 'JetBrains Mono', monospace !important;
}
...
.gradio-accordion {
    ...
    border: 1px solid rgba(42,63,95,0.4) !important;
    border-radius: 12px !important;
    transition: border-color 0.2s !important;
}
.gradio-accordion:hover { border-color: rgba(77,159,255,0.3) !important; }

/* -- HERO SEARCH BOX ----------------------------------------------- */
.search-hero textarea, .search-hero input {
    font-size: 16px !important;
    font-weight: 500 !important;
    border: 2px solid rgba(77,159,255,0.3) !important;
    border-radius: 12px !important;
...
button.primary:hover { opacity:0.92 !important; transform:translateY(-1px) !important; box-shadow:0 6px 20px rgba(99,102,241,0.45) !important; }
button.primary:active { transform:translateY(0) scale(0.98) !important; }
button.primary:disabled { opacity:0.5 !important; cursor:not-allowed !important; transform:none !important; }
...
button.secondary:hover { background:rgba(25,38,60,0.95) !important; border-color:rgba(77,159,255,0.5) !important; color:#C8DCFF !important; }
button.secondary:active { transform:scale(0.97) !important; }

/* -- CHIP BUTTONS — Fixed 130px, even grid ------------------------- */
.chip-row button, button.chip {
    width: 130px !important; flex: 0 0 130px !important;
    ...
    border: 1px solid rgba(77,159,255,0.2) !important;
    color: #8AAED4 !important;
    transition: background 0.15s, border-color 0.2s, transform 0.1s !important;
}
.chip-row button:hover { background:rgba(59,130,246,0.12) !important; border-color:rgba(59,130,246,0.5) !important; color:#E2EAF8 !important; }
.chip-row button:active { transform:scale(0.94) !important; background:rgba(59,130,246,0.2) !important; }

/* -- TABS ---------------------------------------------------------- */
.tab-nav { border-bottom:1px solid rgba(42,63,95,0.5) !important; gap:2px !important; }
.tab-nav button {
    background: transparent !important; color: #5A7A9E !important;
    font-family: 'Space Grotesk','Inter',sans-serif !important;
    font-weight: 500 !important; font-size: 13px !important;
...
.tab-nav button:hover { background:rgba(77,159,255,0.07) !important; color:#94B4D8 !important; }
.tab-nav button.selected {
    background: rgba(77,159,255,0.1) !important; color:#60A5FA !important;
    border-color: rgba(77,159,255,0.2) !important;
    border-bottom: 2px solid #3B82F6 !important; font-weight:600 !important;
}

/* -- STATUS BAR — typed by content --------------------------------- */
/* Success */
.status-success { border-left:3px solid #22C55E !important; background:rgba(34,197,94,0.07) !important; border-radius:0 8px 8px 0 !important; padding:10px 14px !important; animation: slideIn 0.3s ease !important; }
/* Warning */
.status-warning { border-left:3px solid #F59E0B !important; background:rgba(245,158,11,0.07) !important; border-radius:0 8px 8px 0 !important; padding:10px 14px !important; }
/* Error */
.status-error   { border-left:3px solid #EF4444 !important; background:rgba(239,68,68,0.07) !important; border-radius:0 8px 8px 0 !important; padding:10px 14px !important; }

@keyframes slideIn { from { opacity:0; transform:translateY(-6px); } to { opacity:1; transform:translateY(0); } }

/* -- KPI CARDS ----------------------------------------------------- */
/* Applied to the KPI row — each cell becomes a card */
.kpi-card-row { display:flex; gap:10px; margin:8px 0; }
.kpi-card {
    flex:1; background:rgba(15,22,36,0.9) !important;
    border:1px solid rgba(42,63,95,0.5) !important;
    border-radius:12px !important;
    padding: 14px 16px !important;
    transition: transform 0.15s, box-shadow 0.15s, border-color 0.2s !important;
    box-shadow: 0 2px 12px rgba(0,0,0,0.25) !important;
}
.kpi-card:hover { transform:translateY(-2px) !important; box-shadow:0 6px 24px rgba(0,0,0,0.35) !important; border-color:rgba(77,159,255,0.3) !important; }
.kpi-value { font-size:22px !important; font-weight:700 !important; color:#F0F6FF !important; font-family:'Space Grotesk',sans-serif !important; }
.kpi-label { font-size:11px !important; font-weight:500 !important; color:#5A7A9E !important; text-transform:uppercase !important; letter-spacing:0.07em !important; margin-top:3px !important; }
.kpi-sub   { font-size:12px !important; color:#3B82F6 !important; margin-top:2px !important; font-style:italic !important; }

/* -- TABLES -------------------------------------------------------- */
table { width:100% !important; border-collapse:separate !important; border-spacing:0 !important; border-radius:10px !important; overflow:hidden !important; }
th { background:rgba(15,22,36,0.98) !important; color:#60A5FA !important; font-family:'Space Grotesk',sans-serif !important; font-weight:600 !important; font-size:11.5px !important; letter-spacing:0.06em !important; text-transform:uppercase !important; padding:10px 14px !important; border-bottom:1px solid rgba(77,159,255,0.2) !important; }
td { padding:8px 14px !important; border-bottom:1px solid rgba(20,30,50,0.5) !important; font-size:13px !important; color:#CBD8F0 !important; font-family:'Inter',sans-serif !important; }
tr:hover td { background:rgba(77,159,255,0.04) !important; }
tr:last-child td { border-bottom:none !important; }

/* -- DROPDOWNS ----------------------------------------------------- */
select, .gr-dropdown { background:rgba(10,16,28,0.9) !important; color:#E2EAF8 !important; border:1px solid rgba(77,159,255,0.2) !important; border-radius:10px !important; font-family:'Inter',sans-serif !important; }

/* -- PLOT CONTAINERS ----------------------------------------------- */
.gr-plot { border-radius:12px !important; overflow:hidden !important; border:1px solid rgba(42,63,95,0.4) !important; }

/* -- LOADING PROGRESS BAR ------------------------------------------ */
.progress-bar { background:linear-gradient(90deg,#3B82F6,#6366F1) !important; border-radius:4px !important; }

/* -- MOBILE RESPONSIVE --------------------------------------------- */
@media (max-width: 768px) {
    .chip-row { display:grid !important; grid-template-columns:repeat(3,1fr) !important; gap:6px !important; }
    .chip-row button { width:auto !important; flex:none !important; }
    h1 { font-size:20px !important; }
    .tab-nav button { font-size:11px !important; padding:7px 10px !important; }
}

/* -- ACCORDION LABELS ---------------------------------------------- */
.gradio-accordion > .label-wrap { color:#94B4D8 !important; font-family:'Space Grotesk',sans-serif !important; font-weight:500 !important; font-size:13px !important; }
...
/* -- CHIP BUTTONS — Gradio native, styled as pills ─────────── */
.chip-btn button, .chip-row button {
    border-radius: 20px !important;
...
.chip-btn button:hover, .chip-row button:hover {
...
.chip-btn button:active, .chip-row button:active {
    transform: scale(0.95) translateY(0) !important;
}
/* Selected chip state — applied via JS class toggle */
.chip-selected button {
    background: linear-gradient(135deg, rgba(59,130,246,0.35), rgba(99,102,241,0.28)) !important;
    border-color: #3B82F6 !important;
    color: #FFFFFF !important;
    font-weight: 700 !important;
    box-shadow: 0 0 0 2px rgba(59,130,246,0.4), 0 4px 16px rgba(59,130,246,0.3) !important;
...
.chip-row { gap: 6px !important; flex-wrap: wrap !important; margin: 2px 0 !important; }

/* -- FOOTER -------------------------------------------------------- */
footer { display:none !important; }
"""

with gr.Blocks(css=CSS, title="FinIQ v6 — India's First Multilingual AI Financial Intelligence") as demo:
...
<div style="padding:24px 0 8px;">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:4px;">
    ...
    <div>
      <h1 style="margin:0;font-family:'Space Grotesk',sans-serif;font-size:26px;
                 font-weight:700;color:#F0F6FF;letter-spacing:-0.02em;">
        FinIQ v6
        <span style="font-size:14px;font-weight:400;color:#5A7A9E;margin-left:8px;">
          India's First Multilingual AI Financial Intelligence
        </span>
      </h1>
      <p style="margin:4px 0 0;font-size:13px;color:#5A7A9E;font-family:'Inter',sans-serif;">
        🇮🇳 Hindi · বাংলা · मराठी · தமிழ் · తెలుగు · ગુj. &nbsp;·&nbsp;
        Groq LLaMA 3.3 70B &nbsp;·&nbsp; Gemini 2.0 Flash &nbsp;·&nbsp;
        Altman Z · Beneish M · Graham Number &nbsp;·&nbsp; Zero Cost
      </p>
    </div>
  </div>
</div>
""")
    ...
    # ── SEARCH ROW — prominent, first interactive element ────────────────────
    with gr.Row(equal_height=True):
        inp_query = gr.Textbox(
            label="Stock / Ticker",
            placeholder="TCS · Reliance · HDFC Bank · Infosys · AAPL",
            scale=6, lines=1, elem_id="finiq-search")
        btn_fetch = gr.Button("🚀  Fetch & Analyse", variant="primary",
                              scale=2, min_width=160)
        btn_demo  = gr.Button("🎯  Demo", variant="secondary", scale=1, min_width=80)
        btn_reset = gr.Button("🔄  Reset", variant="secondary", scale=1, min_width=80)

    # ── CHIP ROW — fixed-width pills ─────────────────────────────────────────
    # Stocks derived strictly from TICKER_MAP — only verified tickers
    # Only reliable large-cap Nifty 50 stocks — tested to work consistently on Yahoo Finance
    # Removed: Zomato, IRCTC, Coal India, Kotak, Tech Mahindra (rate limit / data gaps)
    STOCKS = [
        "TCS", "Infosys", "Reliance", "HDFC Bank", "Wipro",
        "Bajaj Finance", "Maruti", "ITC", "Airtel", "Sun Pharma",
        "Titan", "ICICI Bank", "SBI", "Axis Bank", "HCL",
        "ONGC", "Asian Paints", "Nestle India", "Tata Motors", "NTPC"
    ]
    # Double-check all are in TICKER_MAP
    STOCKS = [s for s in STOCKS if s.lower() in TICKER_MAP]

    # JS to highlight selected chip and fill search box
    # ── STOCK CHIPS — real Gradio buttons, each triggers fetch directly ────────
    # CSS class .chip-btn is styled in CSS block above
    selected_stock = gr.State(value="")   # tracks which chip is selected

    gr.Markdown("**Popular stocks — click to load instantly:**",
                elem_classes=["chip-label"])

    # Row 1
    with gr.Row(elem_classes=["chip-row"]):
        chip_btns_r1 = [gr.Button(s, variant="secondary", size="sm",
                                   elem_id=f"chip-{s.replace(' ','_')}",
                                   elem_classes=["chip-btn"])
                        for s in STOCKS[:10]]
    # Row 2
    with gr.Row(elem_classes=["chip-row"]):
        chip_btns_r2 = [gr.Button(s, variant="secondary", size="sm",
                                   elem_id=f"chip-{s.replace(' ','_')}",
                                   elem_classes=["chip-btn"])
                        for s in STOCKS[10:]]
    ...
    with gr.Row():
        with gr.Accordion("🎙️  Voice Input", open=False):
            gr.Markdown(
                "Tap mic → speak → click **Stop** → click **Transcribe** → click **Fetch & Analyse**\n\n"
                "*Groq Whisper · All browsers · Requires FinAIKey2*"
            )
            with gr.Row():
                inp_audio      = gr.Audio(sources=["microphone"], type="filepath",
                                          label="🎙️ Record", scale=3)
                btn_transcribe = gr.Button("📝  Transcribe", variant="secondary",
                                           scale=1, min_width=110)
            out_voice_status = gr.Markdown(value="")
            btn_transcribe.click(on_voice_transcribe,
                                 inputs=[inp_audio],
                                 outputs=[inp_query, out_voice_status])

        with gr.Accordion("📁  Upload Excel", open=False):
            with gr.Row():
                inp_excel = gr.File(label="Upload .xlsx / .xls",
                                    file_types=[".xlsx",".xls"], scale=4)
                btn_excel = gr.Button("📊  Analyse", variant="secondary",
                                      scale=1, min_width=120)

    # ── STATUS BAR ────────────────────────────────────────────────────────────
    out_status = gr.Markdown(value="*Enter a stock name above and click **Fetch & Analyse***")
    out_header = gr.Markdown(value="")

    # ── KPI CARDS — key metrics ───────────────────────────────────────────────
    with gr.Accordion("📐  Key Metrics & Risk Scores", open=True):
        out_kpi = gr.Markdown(value="*Fetch a company to see key metrics.*")

    gr.HTML("<div style='height:4px;background:linear-gradient(90deg,#3B82F6,#6366F1,#14B8A6);border-radius:2px;margin:8px 0 16px;opacity:0.5;'></div>")

    # ── GROUPED TABS (5 groups, no overflow) ─────────────────────────────────
    with gr.Tabs():
        ...
        with gr.TabItem("📈  Analysis"):
            with gr.Tabs():
                ...
                with gr.TabItem("📊  Margins"):
                    gr.Markdown("**Gross · EBITDA · Net margin trends**")
                    fig_mar = gr.Plot(show_label=False)
                with gr.TabItem("📉  EPS Trend"):
                    gr.Markdown("**Earnings per share with P/E reference**")
                    fig_eps = gr.Plot(show_label=False)
                with gr.TabItem("🌊  Waterfall"):
                    gr.Markdown("**Revenue → COGS → Gross Profit → Net Income bridge**")
                    fig_wf  = gr.Plot(show_label=False)
                with gr.TabItem("🏦  Balance Sheet"):
                    gr.Markdown("**Assets · Debt · Equity over time**")
                    fig_bs  = gr.Plot(show_label=False)
                with gr.TabItem("💵  Cash Flow"):
                    gr.Markdown("**Operating CF · Free CF · CapEx**")
                    fig_cf  = gr.Plot(show_label=False)
                with gr.TabItem("🔗  DuPont"):
                    gr.Markdown("**ROE decomposition: Margin × Turnover × Leverage**")
                    fig_dp  = gr.Plot(show_label=False)
                with gr.TabItem("🌡️  DCF"):
                    gr.Markdown("**EV/FCF sensitivity heatmap**")
                    fig_dcf = gr.Plot(show_label=False)
                with gr.TabItem("🎯  Gauges"):
                    gr.Markdown("**Key ratio gauge charts**")
                    fig_rg  = gr.Plot(show_label=False)
                with gr.TabItem("📅  Price"):
                    gr.Markdown("**1-year candlestick + SMA 50/200 + Volume**")
                    fig_ph  = gr.Plot(show_label=False)
                with gr.TabItem("🧭  Radar"):
                    gr.Markdown("**Multi-dimensional financial sentiment**")
                    fig_sd  = gr.Plot(show_label=False)

        # ════════ GROUP 2: RISK & SIGNALS ════════
        with gr.TabItem("⚠️  Risk & Signals"):
            with gr.Tabs():
                ...
                with gr.TabItem("🚩  Red Flags"):
                    gr.Markdown("**Automated red flag scorecard by severity**")
                    fig_rf = gr.Plot(show_label=False)
                with gr.TabItem("🧠  Bias Detector"):
                    gr.Markdown(
                        "### 🧠 Investor Bias Detector\n"
                        "Rule-based analysis flags **behavioural traps** — no AI call, instant on fetch.\n\n"
                        "> 🏆 **Zero competitor in India has this feature.** "
                        "First tool combining quantitative + behavioural finance.\n\n"
                        "*Kahneman & Tversky (1979). Prospect Theory*"
                    )
                    out_bias = gr.Markdown(value="*Fetch a company to see bias analysis.*")
                with gr.TabItem("📈  vs Nifty 50"):
                    gr.Markdown("**1/3/5-year returns vs Nifty 50 index benchmark**")
                    btn_nifty = gr.Button("📊  Compare vs Nifty 50", variant="primary")
                    out_nifty = gr.Markdown(value="*Fetch an NSE stock first.*")
                    btn_nifty.click(on_nifty_compare, inputs=[], outputs=[out_nifty])

        # ════════ GROUP 3: AI INTELLIGENCE ════════
        with gr.TabItem("🤖  AI Intelligence"):
            with gr.Tabs():
                with gr.TabItem("🤖  AI Analysis (6 Languages)"):
                    gr.Markdown(
                        "### 🤖 AI-Powered Analysis — 6 Indian Languages\n"
                        "**Groq LLaMA 3.3 70B** → 6 sections: Financial Analysis · "
                        "Peer Benchmarking · Strengths · Risks · 3-Year Forecast · Verdict\n\n"
                        "> 🏆 First financial AI in India: **Hindi · বাংলা · मराठी · தமிழ் · తెలుగు · ગુj.**"
                    )
                    with gr.Row():
                        dd_lang = gr.Dropdown(
                            choices=["English","Hindi","Bengali","Marathi","Tamil","Telugu","Gujarati"],
                            value="English", label="🌐 Language", scale=1)
                        btn_ai  = gr.Button("🚀  Generate AI Analysis", variant="primary", scale=3)
                    out_ai = gr.Markdown(value="*Select a language and click Generate.*")

                with gr.TabItem("🏆  Peer Benchmarking"):
                    gr.Markdown("**Sector benchmarks + AI peer positioning**")
                    out_peer = gr.Markdown(value="*Fetch data first.*")

                with gr.TabItem("📅  3-Year Forecast"):
                    gr.Markdown("**Bear · Base · Bull scenarios + key metrics to watch**")
                    out_forecast = gr.Markdown(value="*Fetch data first.*")

                with gr.TabItem("📰  News Sentiment"):
                    gr.Markdown("**NLP news scoring + sentiment distribution**")
                    btn_news = gr.Button("📰  Load News Sentiment", variant="primary")
                    out_news_summary = gr.Markdown(value="")
                    with gr.Row():
                        with gr.Column(scale=1): plt_news = gr.Plot(label="Sentiment")
                ...
                with gr.TabItem("💬  Q&A"):
                    gr.Markdown("**Ask anything — answers grounded in loaded data only**")
                    inp_qa = gr.Textbox(label="Your question",
                                        placeholder="Which year had best margins? · Is debt rising? · What drives ROE?",
                                        lines=2)
                ...
                with gr.TabItem("📖  Learn"):
                    gr.Markdown(
                        ...
                        "AI explains any metric in plain language — in 6 Indian languages.\n\n"
                        "*🎓 FinIQ teaches as it analyses*"
                    )
                    with gr.Row():
                        dd_metric  = gr.Dropdown(choices=EXPLAIN_METRICS,
                                                 label="Select metric", scale=2)
                        dd_explang = gr.Dropdown(
                            choices=["English","Hindi","Bengali","Marathi","Tamil","Telugu","Gujarati","Simple English (beginner)"],
                            value="English", label="Language", scale=1)
                        btn_explain_m = gr.Button("📖 Explain", variant="primary",
                                                  scale=1, min_width=100)
                    out_metric_explain = gr.Markdown(value="*Select a metric and click Explain.*")
                    btn_explain_m.click(on_explain_metric,
                                        inputs=[dd_metric, dd_explang],
                                        outputs=[out_metric_explain])

        # ════════ GROUP 4: COMPARE & PORTFOLIO ════════
        with gr.TabItem("⚔️  Compare & Portfolio"):
            with gr.Tabs():
                with gr.TabItem("⚔️  Compare 2–3 Companies"):
                    gr.Markdown(
                        "### ⚔️ Head-to-Head Comparison\n"
                        "**11 metrics · 6-axis radar · 🏆 Best-in-category · AI narrative**\n\n"
                        "*Closes the biggest gap vs Screener.in, Tickertape, Tijori Finance*"
                    )
                    _cmp_stocks = ["TCS","Infosys","Reliance","HDFC Bank","Wipro",
                                   "Bajaj Finance","Maruti","ITC","Zomato","Airtel",
                                   "Sun Pharma","Titan","ONGC","SBI","Axis Bank",
                                   "Kotak Bank","HUL","Nestle","Tata Motors","L&T","NTPC"]
                    with gr.Row():
                        ...
                        btn_cmp   = gr.Button("⚔️  Compare", variant="primary",
                                              scale=1, min_width=120)
                    out_cmp_table = gr.Dataframe(label="Side-by-Side", wrap=True, interactive=False)
                    out_cmp_radar = gr.Plot(show_label=False)
                    out_cmp_md    = gr.Markdown(value="*Select companies and click Compare.*")
                    btn_cmp.click(on_compare,
                                  inputs=[inp_cmp_a, inp_cmp_b, inp_cmp_c],
                                  outputs=[out_cmp_table, out_cmp_radar, out_cmp_md])

                with gr.TabItem("📊  Portfolio Risk (2–5 Stocks)"):
                    gr.Markdown(
                        "### 📊 Portfolio Risk Aggregator\n"
                        "**Blended Altman Z · Sector allocation · Distress flags · AI commentary**\n\n"
                        "*Not available in any other free Indian financial tool*"
                    )
                    _port_stocks = ["TCS.NS","RELIANCE.NS","INFY.NS","HDFCBANK.NS","WIPRO.NS",
                                    ...
                                    "LT.NS","NTPC.NS","POWERGRID.NS","HEROMOTOCO.NS","BPCL.NS"]
                    with gr.Row():
                    ...
                    with gr.Row():
                        ...
                        btn_port = gr.Button("📊  Analyse Portfolio", variant="primary",
                                             scale=2, min_width=160)
                    with gr.Row():
                        out_port_z   = gr.Plot(label="Altman Z", scale=1)
                        out_port_pie = gr.Plot(label="Sectors",   scale=1)
                    out_port_md = gr.Markdown(value="*Select 2-5 stocks and click Analyse.*")
                    btn_port.click(on_portfolio,
                                   inputs=[inp_p1, inp_p2, inp_p3, inp_p4, inp_p5],
                                   outputs=[out_port_z, out_port_pie, out_port_md])

        # ════════ GROUP 5: STATEMENTS ════════
        with gr.TabItem("📋  Statements"):
            with gr.Row():
                rad_period = gr.Radio(["Annual","Quarterly","TTM"], value="Annual",
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("💸  Cash Flow"):
                    ...
                    out_cf  = gr.Dataframe(label="Cash Flow", wrap=True, interactive=False)
            btn_stmts.click(on_statements,
                            inputs=[rad_period, sld_rows],
                            outputs=[out_inc, out_bal, out_cf, out_stmt_status,
                                     out_inc_ins, out_bal_ins, out_cf_ins])

        # ════════ GROUP 6: TOOLS & EXPORT ════════
        with gr.TabItem("🔧  Tools & Export"):
            with gr.Tabs():
                with gr.TabItem("💡  Impact"):
                    btn_impact = gr.Button("📊  Load Impact Dashboard", variant="primary")
                    out_impact = gr.Markdown(value="*Click to load.*")
                    btn_impact.click(on_impact, inputs=[], outputs=[out_impact])

                with gr.TabItem("🔍  Explainability"):
                    gr.Markdown(
                        "### 🔍 AI Transparency — Every Score's Computation\n"
                        "Altman Z component breakdown · Beneish M indices · "
                        "Ratio drivers with raw numerators/denominators\n\n"
                        "*Altman (1968) · Beneish (1999) · Graham & Dodd (1934)*"
                    )
                    btn_explain = gr.Button("🔬  Show Breakdown", variant="primary")
                    out_explain = gr.Markdown(value="*Fetch a company first.*")
                    btn_explain.click(on_explain, inputs=[], outputs=[out_explain])

                with gr.TabItem("📥  Export"):
                    gr.Markdown("### 📥 Export Your Analysis")
                    with gr.Row():
                        with gr.Column(scale=1):
                            gr.Markdown("**📊 Excel Dashboard** — Charts + KPI cards + dark theme")
                            btn_export     = gr.Button("📊  Download Excel Dashboard", variant="primary")
                            out_exp_status = gr.Markdown(value="")
                            out_exp_file   = gr.File(label="Excel Dashboard", visible=False)
                            btn_export.click(on_export, inputs=[],
                                             outputs=[out_exp_file, out_exp_status])
                            out_exp_file.change(lambda f: gr.File(visible=f is not None),
                                                inputs=[out_exp_file], outputs=[out_exp_file])
                        with gr.Column(scale=1):
                            gr.Markdown("**🏆 Premium Report** — Cover page + 4 chart sheets + Risk + AI summary")
                            btn_premium    = gr.Button("🏆  Download Premium Report", variant="primary")
                            out_prem_status= gr.Markdown(value="")
                            out_prem_file  = gr.File(label="Premium Report", visible=False)
                            btn_premium.click(on_premium_export, inputs=[],
                                              outputs=[out_prem_file, out_prem_status])
                            out_prem_file.change(lambda f: gr.File(visible=f is not None),
                                                 inputs=[out_prem_file], outputs=[out_prem_file])
                        with gr.Column(scale=1):
                            gr.Markdown("**📄 PDF Brief** — 2-page professional brief")
                            btn_pdf        = gr.Button("📄  Download PDF Brief", variant="secondary")
                            out_pdf_status = gr.Markdown(value="")
                            out_pdf_file   = gr.File(label="PDF", visible=False)
                            btn_pdf.click(on_pdf_export, inputs=[],
                                          outputs=[out_pdf_file, out_pdf_status])
                            out_pdf_file.change(lambda f: gr.File(visible=f is not None),
                                                inputs=[out_pdf_file], outputs=[out_pdf_file])

                with gr.TabItem("📱  Share"):
                    gr.Markdown(
                        "### 📱 Share on WhatsApp\n"
                        "> 🇮🇳 WhatsApp is how 90% of Indian retail investors share tips — "
                        "FinIQ makes your analysis shareable in one click."
                    )
                    btn_wa = gr.Button("📱  Generate WhatsApp Message", variant="primary")
                    out_wa = gr.HTML(value="*Fetch a company first.*")
                    btn_wa.click(on_whatsapp_share, inputs=[], outputs=[out_wa])

                with gr.TabItem("ℹ️  How It Works"):
                    gr.Markdown("""### ℹ️ How FinIQ v6 Works

**Flow:** `Query → Local Ticker Map → Yahoo Finance (Chrome session) → Smart Cache → 15 Charts → Groq LLaMA 3.3 70B → 6-Language Analysis`
...
- **Graham & Dodd (1934)** — Graham Number intrinsic value · *Security Analysis*
- **Kahneman & Tversky (1979)** — Behavioural bias framework · *Prospect Theory*

| Feature | FinIQ v6 | Screener.in | Tickertape | Bloomberg |
|---------|----------|-------------|------------|-----------|
| Indian languages | **6** | 0 | 0 | 0 |
| Behavioural bias | ✅ | ❌ | ❌ | ❌ |
| Voice input | ✅ | ❌ | ❌ | ❌ |
| SEBI flag | ✅ | ❌ | ❌ | ✅ paid |
| Explainability | ✅ | ❌ | ❌ | ❌ |
| Cost | **₹0** | ₹999/mo | ₹499/mo | ₹18L/yr |
""")
...
<div style="margin-top:24px;padding:16px;
            ...
            border:1px solid rgba(59,130,246,0.15);border-radius:12px;text-align:center;">
  <p style="margin:0;font-size:12px;color:#5A7A9E;font-family:'Inter',sans-serif;line-height:1.8;">
    <strong style="color:#8AAED4;">FinIQ v6</strong> &nbsp;·&nbsp;
    🇮🇳 India's First Multilingual AI Financial Platform &nbsp;·&nbsp;
    Hindi · বাংলা · मराठी · தமிழ் · తెలుగు · ગુj. &nbsp;·&nbsp;
    Groq LLaMA 3.3 70B &nbsp;·&nbsp; Gemini 2.0 Flash Lite &nbsp;·&nbsp;
    Altman (1968) · Beneish (1999) · Graham & Dodd (1934) &nbsp;·&nbsp;
...
""")
    ...
    _fetch_outs = [out_status, out_header, out_kpi, out_peer,
                   *_all_figs, out_forecast, out_bias]
    ...
    # Wire chip buttons — each chip sets inp_query value then triggers fetch
    # Use gr.State to pass stock name cleanly
    for chip_btn, stock_name in zip(chip_btns, STOCKS):
        chip_btn.click(
            fn=lambda s=stock_name: s,
            inputs=[],
            outputs=[inp_query]
        ).then(
            fn=on_fetch,
    ...
    # Also allow pressing Enter in search box to fetch
    inp_query.submit(on_fetch, inputs=[inp_query], outputs=_fetch_outs,
                     show_progress="full")

    # JS to highlight selected chip visually (CSS class swap)
    gr.HTML(f"""<script>
    // Listen for Gradio button clicks in chip rows and highlight the active one
    document.addEventListener('DOMContentLoaded', function() {{
        function wireChips() {{
            var chipRows = document.querySelectorAll('.chip-row');
            chipRows.forEach(function(row) {{
                row.querySelectorAll('button').forEach(function(btn) {{
                    btn.addEventListener('click', function() {{
                        // Reset all chip buttons
                        document.querySelectorAll('.chip-row button').forEach(function(b) {{
                            b.style.background   = '';
                            b.style.borderColor  = '';
                            b.style.color        = '';
                            b.style.fontWeight   = '';
                        ...
                        btn.style.background  = 'linear-gradient(135deg,rgba(59,130,246,0.35),rgba(99,102,241,0.28))';
                        btn.style.borderColor = '#3B82F6';
                        btn.style.color       = '#FFFFFF';
                        btn.style.fontWeight  = '700';
                        btn.style.boxShadow   = '0 0 0 2px rgba(59,130,246,0.4),0 4px 16px rgba(59,130,246,0.3)';
                        btn.style.transform   = 'scale(1.05)';
                    }});
                }});
            }});
        }}
        // Try immediately and retry after Gradio renders
        setTimeout(wireChips, 1000);
        setTimeout(wireChips, 3000);
    }});
    </script>""")
