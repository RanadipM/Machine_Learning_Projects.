...
[data-testid="stSidebar"] {
    background: linear-gradient(180deg, #0D1B4B 0%, #1E2761 60%, #0D1B4B 100%);
    border-right: 1px solid #2d3a7a;
}
...
[data-testid="stSidebar"] hr { border-color: #2d3a7a !important; }
...
h2 { font-size: 1.3rem !important; font-weight: 600 !important; color: #1E2761 !important; }
h3 { font-size: 1.1rem !important; font-weight: 600 !important; color: #1E2761 !important; }

/* ── Metric cards ── */
[data-testid="metric-container"] {
    background: linear-gradient(135deg, #0D1B4B, #1E2761);
    border-radius: 12px;
    padding: 16px 20px;
...
[data-testid="metric-container"]:hover {
    ...
    box-shadow: 0 8px 25px rgba(13,27,75,0.3);
}
...
[data-testid="metric-container"] [data-testid="stMetricDelta"] { color: #4FC3F7 !important; }
...
/* ── Buttons ── */
.stButton > button {
    background: linear-gradient(135deg, #1E2761, #2d6a9f);
    color: white;
    border: none;
    border-radius: 8px;
    font-weight: 600;
    ...
    box-shadow: 0 3px 10px rgba(30,39,97,0.3);
}
.stButton > button:hover {
    background: linear-gradient(135deg, #4FC3F7, #1E2761);
    transform: translateY(-1px);
    box-shadow: 0 6px 18px rgba(30,39,97,0.4);
}
.stButton > button[kind="primary"] {
    background: linear-gradient(135deg, #4FC3F7, #0891B2);
    font-size: 15px;
    padding: 12px 24px;
}
...
[data-testid="stSlider"] > div > div > div {
    background: linear-gradient(90deg, #4FC3F7, #0891B2) !important;
}
...
/* ── Page header banner ── */
.page-header {
    background: linear-gradient(135deg, #0D1B4B, #1E2761, #1C7293);
    border-radius: 14px;
    padding: 20px 28px;
    margin-bottom: 24px;
    box-shadow: 0 4px 20px rgba(13,27,75,0.3);
    border-left: 5px solid #4FC3F7;
}
.page-header h1 { color: white !important; margin: 0 !important; font-size: 1.8rem !important; }
.page-header p  { color: #CADCFC !important; margin: 4px 0 0 0 !important; font-size: 13px; }
...
.section-card:hover { box-shadow: 0 6px 20px rgba(0,0,0,0.10); }
</style>
...
if page == "📊 Portfolio Overview":
    ...
    <div class="page-header">
        ...
        <p>Real Lending Club data · 100,000 loans · Azure SQL Database · ADLS Gen2</p>
    </div>
    """, unsafe_allow_html=True)
...
# ── PAGE 2: Risk Analysis ────────────────────────────────────────────────────
    ...
    <div class="page-header">
        <h1>📈 Risk Analysis</h1>
        <p>PD scores · Expected loss · Interest rate trends · Grade-level breakdown</p>
    </div>
    """, unsafe_allow_html=True)
...
# ── PAGE 3: Loan Explorer ────────────────────────────────────────────────────
    ...
    <div class="page-header">
        <h1>🔍 Loan Explorer</h1>
        <p>Filter and explore individual loans · Top 500 by expected loss · Interactive charts</p>
    </div>
    """, unsafe_allow_html=True)
...
# ── PAGE 4: ML Scoring ───────────────────────────────────────────────────────
    ...
    st.markdown("""
    <div class="page-header">
        <h1>🤖 ML Credit Risk Scorer</h1>
        <p>Random Forest model · Trained on 100K Lending Club loans · ROC-AUC: 0.688 · 4 models compared</p>
    </div>
    """, unsafe_allow_html=True)
...
# ── PAGE 5: Pipeline Status ──────────────────────────────────────────────────
    ...
    <div class="page-header">
        ...
        <p>Azure Functions · ADF orchestration · Data quality · Table counts</p>
    </div>
    """, unsafe_allow_html=True)
