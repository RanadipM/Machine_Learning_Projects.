...
# ========================= ASSISTANT =========================
with tab_chat:
    # Course catalog — always available so students see what's offered.
    # Expanded by default before any conversation; collapsible afterwards.
    with st.expander("🎓 Programs we offer", expanded=not st.session_state.history):
        theme.course_catalog()
    ...
    SUGGESTIONS = {
        ...
        "Admin": [
            ...
            "How are the network tiers separated by firewall rules?",
            "What happens when a user securely connects over HTTPS/TLS?",
            "How would we detect a DDoS attack on the API?",
            "How does the system prevent prompt injection?",
            "What security controls protect student and employer data?",
    ...
    for r0 in range(0, len(sugg), 2):
        ...
        for j, ex in enumerate(sugg[r0:r0 + 2]):
            if cols[j].button(ex, key=f"sugg_{role}_{r0+j}", use_container_width=True):
                # The 🔒 prefix marks a question that intentionally demonstrates
                # an RBAC denial; strip it so the agent receives a clean query.
                st.session_state.pending_q = ex.replace("🔒", "").strip()
        ...
        if not gi["allowed"]:
            ...
            with st.chat_message("assistant"):
                ...
                if result.get("denied"):
                    log_event("RBAC_DENY",
                              f"role={role} blocked from agent={result['category']}")
                else:
