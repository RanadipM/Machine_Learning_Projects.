...
_AGENT_PERSONA = {
    ...
    "career": "the Career Guidance agent. Give personalized, encouraging, realistic career advice. The student may want a data/AI career OR a core-engineering, higher-studies, government-exam, product, or startup path. Respect their stated goal — do NOT push everyone toward data science. If their goal is outside the knowledge base, give sound general guidance (skills to build, typical roles, next steps) and note that our programs can complement that path.",
    "system": "the System & Security agent. Explain how the platform protects data, controls access, monitors activity, and handles security — accurately and clearly for staff/admin users.",
}
...
def route(query: str) -> tuple[str, dict]:
    ...
    if any(c in q for c in ["pgpds","ai engineering","cybersecurity","business analytics"]):
        if "eligible" in q or "apply" in q or "admission" in q:
            best = "course"
    # access control / roles / permissions -> system (security), not course
    if ("access control" in q or "controlled across" in q
...
def run(query: str, role: str = "Student", profile: Optional[dict] = None) -> dict:
    ...
    if category == "system":
        # scope to system/security docs first so they aren't crowded out
