    ...
    if category in ("course", "placement", "career") and has_profile:
        ...
        if category == "course":
            extra = (
                ...
                "IMPORTANT: Do not just list all courses generically. Recommend the SINGLE "
                "best-fit program for THIS student given their background, interest, and goal, "
                "explain WHY it fits them specifically, and briefly mention one alternative. "
                "Connect the recommendation to their stated background and goal by name."
            )
        elif category == "placement":
            extra = (
                f"\n\nThe student's profile: academic background='{bg}', "
                f"interest='{interest}', career goal='{goal}'. "
                "Tailor placement guidance to roles and outcomes relevant to THIS student's "
                "goal and background; reference their goal by name."
            )
        else:  # career
            extra = (
                f"\n\nThe student's profile: academic background='{bg}', "
                f"interest='{interest}', career goal='{goal}'. "
                "Give personalized, specific advice for THIS profile — respect their stated "
                "goal (do NOT default everyone to data science), reference their background "
                "and goal by name, and give concrete next steps."
            )
        trace["steps"].append(
            ("Personalization", f"{category.title()} agent tailored the answer to the student profile "
