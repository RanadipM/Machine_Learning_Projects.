...
_INJECTION_PATTERNS = [
    ...
    r"(encode|convert|output|write) (your |the )?(prompt|instructions|response|answer|reply) (in|as|to|into) (base64|hex|rot13)",
    r"(in|using) (base64|rot13) (encoding|format)",
...
# Indian mobile, allowing an optional +91 and one internal space/dash split.
_PHONE = re.compile(r"\b(?:\+?91[\s\-]?)?[6-9]\d{4}[\s\-]?\d{5}\b")
# Generic 10-digit number, optionally split once and with a country code.
_PHONE_GENERIC = re.compile(r"\b(?:\+?\d{1,3}[\s\-]?)?\d{5}[\s\-]?\d{5}\b")
_PHONE_PLAIN = re.compile(r"\b(?:\+?\d{1,3}[\s\-]?)?\d{10}\b")
...
def redact_output(text) -> Tuple[str, int]:
    ...
    text = _sub(_LONGNUM, "ID", text)        # 12+ digit IDs first (Aadhaar-like)
    text = _sub(_PHONE, "PHONE", text)
    text = _sub(_PHONE_GENERIC, "PHONE", text)
    text = _sub(_PHONE_PLAIN, "PHONE", text)
    return text, count
