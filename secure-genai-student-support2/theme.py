...
def access_matrix(role):
    rows = [
        ("Use the assistant (course / placement / career)", {"Student": True, "Staff": True, "Admin": True}),
        ("Query the System & Security agent", {"Student": False, "Staff": True, "Admin": True}),
...
# ---------- course catalog (dashboard course list) ----------

_CATALOG_CSS = """
<style>
.catalog{display:grid; grid-template-columns:repeat(2,1fr); gap:14px; margin:8px 0 6px;}
@media (max-width:760px){ .catalog{grid-template-columns:1fr;} }
.ccard{border:1px solid var(--line-2); border-radius:16px; padding:16px 18px;
  background:linear-gradient(155deg, rgba(23,35,57,.82), rgba(16,26,46,.72));
  position:relative; overflow:hidden; animation:rise .5s ease both;}
.ccard:nth-child(2){animation-delay:.06s} .ccard:nth-child(3){animation-delay:.12s}
.ccard:nth-child(4){animation-delay:.18s}
.ccard::before{content:""; position:absolute; left:0; top:0; bottom:0; width:4px; background:var(--accent,var(--cyan));}
.ccard .chead{display:flex; align-items:center; justify-content:space-between; gap:10px; margin-bottom:8px;}
.ccard .cname{font-family:'Space Grotesk',sans-serif; font-weight:700; font-size:15px; color:var(--ink); line-height:1.25;}
.ccard .cdur{font-family:'JetBrains Mono',monospace; font-size:10.5px; letter-spacing:.04em;
  color:var(--ink); background:rgba(46,230,198,.12); border:1px solid var(--line-2);
  padding:4px 9px; border-radius:999px; white-space:nowrap;}
.ccard .cdesc{font-size:12.5px; color:var(--muted); line-height:1.55; margin-bottom:10px;}
.ccard .cmeta{display:flex; flex-wrap:wrap; gap:6px;}
.ccard .ctag{font-size:10.5px; color:var(--muted); background:rgba(124,108,255,.10);
  border:1px solid var(--line); border-radius:7px; padding:3px 8px;}
.ccard .cfee{font-family:'Space Grotesk',sans-serif; font-weight:600; font-size:12.5px; color:var(--cyan); margin-top:9px;}
.catalog-note{font-size:11.5px; color:var(--faint); margin:2px 0 4px; line-height:1.5;}
</style>
"""

# Canonical course catalog — kept in sync with the knowledge base (course-* entries).
_COURSES = [
    {
        "name": "Post Graduate Program in Data Science (PGPDS)",
        "duration": "9 months",
        "accent": "var(--cyan)",
        "desc": "Flagship hybrid program covering the full data-science stack, ending in a capstone project.",
        "tags": ["Python", "SQL", "Machine Learning", "Deep Learning", "NLP", "Time Series"],
        "fit": "Best for freshers or career-switchers wanting a comprehensive foundation.",
        "fee": "₹1,20,000 · 3 installments",
    },
    {
        "name": "AI Engineering Track",
        "duration": "4 months",
        "accent": "var(--sky)",
        "desc": "Advanced program for building modern LLM and agentic AI systems end to end.",
        "tags": ["LLMs", "RAG", "LangChain", "LangGraph", "Multi-agent", "Vector DBs"],
        "fit": "Best if you already know Python and want to specialize in applied AI.",
        "fee": "₹60,000 · 2 installments",
    },
    {
        "name": "Cybersecurity Fundamentals",
        "duration": "8 weeks",
        "accent": "var(--amber)",
        "desc": "Evening program on securing systems and networks — no coding background required.",
        "tags": ["Network Security", "TLS/HTTPS", "RBAC", "OWASP Top 10", "SIEM", "Threat Modeling"],
        "fit": "Best for IT staff or anyone moving into security roles.",
        "fee": "₹25,000 · one-time or 2-part",
    },
    {
        "name": "Business Analytics Program",
        "duration": "3 months",
        "accent": "var(--rose)",
        "desc": "Weekend program on data-driven decision-making with no-code analytics tools.",
        "tags": ["Excel (advanced)", "Power BI", "Tableau", "SQL", "Business Stats"],
        "fit": "Best for business or finance roles that use data without writing code.",
        "fee": "₹35,000 · one-time or 2-part",
    },
]


def course_catalog():
    """Render the four programs the organization offers, as a card grid."""
    st.markdown(_CATALOG_CSS, unsafe_allow_html=True)
    cards = ""
    for c in _COURSES:
        tags = "".join(f'<span class="ctag">{t}</span>' for t in c["tags"])
        cards += (
            f'<div class="ccard" style="--accent:{c["accent"]}">'
            f'<div class="chead"><div class="cname">{c["name"]}</div>'
            f'<div class="cdur">{c["duration"]}</div></div>'
            f'<div class="cdesc">{c["desc"]}</div>'
            f'<div class="cmeta">{tags}</div>'
            f'<div class="cfee">{c["fit"]}</div>'
            f'<div class="catalog-note">Fee: {c["fee"]}</div>'
            f'</div>'
        )
    st.markdown(f'<div class="catalog">{cards}</div>', unsafe_allow_html=True)
    st.markdown(
        '<div class="catalog-note">Ask the assistant below for full curriculum, '
        'eligibility, fees, EMI options, or which program fits your background.</div>',
        unsafe_allow_html=True,
    )
