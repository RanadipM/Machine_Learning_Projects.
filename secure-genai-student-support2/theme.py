...
:root{
  --bg:#070d18; --bg-2:#0b1322; --panel:#101b30; --panel-2:#0e1830;
  --line:rgba(120,160,210,.14); --line-2:rgba(120,160,210,.22);
  --ink:#eaf1fa; --muted:#94a8c4; --faint:#5f74a0;
  --cyan:#35e0d2; --sky:#3aa6f6; --amber:#f6b73c; --rose:#f87a8a;
  --green:#46e08a; --violet:#9d8cf5;
}
...
.stApp{
  background:
    radial-gradient(1100px 600px at 8% -10%, rgba(58,166,246,.12), transparent 55%),
    radial-gradient(1000px 560px at 100% -6%, rgba(53,224,210,.10), transparent 52%),
    radial-gradient(800px 800px at 50% 120%, rgba(157,140,245,.06), transparent 60%),
    linear-gradient(180deg,#070d18 0%, #0a1120 100%);
  color:var(--ink);
}
.stApp::before{
...
h1,h2,h3,h4{font-family:'Space Grotesk',sans-serif; letter-spacing:-.015em; color:var(--ink);}
...
.hero{position:relative; overflow:hidden; border:1px solid var(--line-2); border-radius:22px;
  ...
  background:
    radial-gradient(640px 220px at 90% -30%, rgba(53,224,210,.20), transparent 62%),
    radial-gradient(520px 220px at 10% 120%, rgba(58,166,246,.16), transparent 60%),
    linear-gradient(135deg, rgba(18,30,52,.94), rgba(11,19,34,.94));
...
.hero h1{font-size:37px; line-height:1.05; margin:14px 0 10px; font-weight:700;}
.hero h1 .grad{background:linear-gradient(92deg,var(--cyan),var(--sky));
  -webkit-background-clip:text; background-clip:text; -webkit-text-fill-color:transparent;}
.hero p.sub{color:var(--muted); font-size:15px; margin:0; max-width:720px; line-height:1.55;}
.pillrow{margin-top:18px; display:flex; flex-wrap:wrap; gap:8px;}
.pill{font-family:'JetBrains Mono',monospace; font-size:11px; letter-spacing:.03em; color:var(--ink);
  background:rgba(53,224,210,.07); border:1px solid var(--line); padding:5px 12px; border-radius:999px;
...
/* STAT GRID */
.statgrid{display:grid; grid-template-columns:repeat(4,1fr); gap:13px; margin:14px 0 4px;}
.stat{position:relative; border:1px solid var(--line); border-radius:16px; padding:16px 18px;
...
.stat::before{content:""; position:absolute; left:0; top:0; bottom:0; width:3px;
  background:linear-gradient(180deg,var(--cyan),var(--sky)); opacity:.9;}
.stat:hover{transform:translateY(-4px); border-color:var(--line-2);
  box-shadow:0 18px 40px -26px rgba(53,224,210,.5);}
.stat .num{font-family:'Space Grotesk',sans-serif; font-weight:700; font-size:26px;
  background:linear-gradient(92deg,var(--cyan),var(--sky));
  -webkit-background-clip:text; background-clip:text; -webkit-text-fill-color:transparent;}
.stat .lbl{font-size:11.5px; color:var(--muted); letter-spacing:.02em; margin-top:3px;}
...
.pipe-step.act{border-color:rgba(53,224,210,.6); background:rgba(53,224,210,.1);
  box-shadow:0 0 0 3px rgba(53,224,210,.1); animation:glow 1.4s ease-in-out infinite;}
.pipe-step.act .k{color:var(--cyan);}
@keyframes glow{0%,100%{box-shadow:0 0 0 3px rgba(53,224,210,.08);} 50%{box-shadow:0 0 0 5px rgba(53,224,210,.16);}}

/* TABS */
.stTabs [data-baseweb="tab-list"]{gap:6px; background:rgba(16,27,48,.55); padding:6px;
  border-radius:14px; border:1px solid var(--line);}
.stTabs [data-baseweb="tab"]{height:44px; border-radius:10px; color:var(--muted);
  font-family:'Space Grotesk',sans-serif; font-weight:600; font-size:14px; padding:0 18px;
  background:transparent; transition:.2s;}
.stTabs [data-baseweb="tab"]:hover{color:var(--ink); background:rgba(53,224,210,.06);}
.stTabs [aria-selected="true"]{
  background:linear-gradient(135deg, rgba(53,224,210,.22), rgba(58,166,246,.18))!important;
  color:#fff!important; border:1px solid rgba(53,224,210,.4);}
...
.stTextInput input:focus, .stChatInput textarea:focus{
  border-color:var(--cyan)!important; box-shadow:0 0 0 3px rgba(53,224,210,.14)!important;}

/* BUTTONS */
.stButton>button{background:linear-gradient(135deg, rgba(53,224,210,.14), rgba(58,166,246,.1));
  color:var(--ink); border:1px solid var(--line); border-radius:11px;
  font-family:'Inter',sans-serif; font-weight:500; font-size:12.5px; text-align:left;
  transition:.22s; box-shadow:0 6px 18px -14px rgba(53,224,210,.6); padding:9px 13px;}
.stButton>button:hover{transform:translateY(-2px); border-color:rgba(53,224,210,.55);
  background:linear-gradient(135deg, rgba(53,224,210,.26), rgba(58,166,246,.18)); color:#fff;}
...
def hero(live=True):
    badge = (
        '<div class="live-badge"><span class="dot"></span>LIVE · GROQ</div>'
        if live else
        '<div class="live-badge" style="color:var(--amber);border-color:rgba(246,183,60,.4);'
        'background:rgba(246,183,60,.08)"><span class="dot" style="background:var(--amber)"></span>MOCK MODE</div>'
    )
    st.markdown(
        f"""
        <div class="hero">
          {badge}
          <span class="eyebrow">AI &middot; Cybersecurity &middot; System Design</span>
          <h1>Secure <span class="grad">GenAI Student-Support</span> Platform</h1>
          <p class="sub">A multi-agent + RAG assistant with security engineered into the request
          path &mdash; input defense, RBAC, PII redaction, and live audit logging. Built, deployed, and
          demonstrable on Streamlit Cloud &amp; HuggingFace Spaces.</p>
          <div class="pillrow">
            ...
            <span class="pill">Zero-Trust Network</span>
