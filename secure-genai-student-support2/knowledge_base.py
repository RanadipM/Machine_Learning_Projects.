...
KNOWLEDGE_BASE = [
    ...
    },
    {
        ...
        "text": (
            "PGPDS: any bachelor's degree, no coding required, minimum 50% aggregate. (This covers whether you are qualified, whether you qualify, and who can join or enroll.)  "
    ...
    },
    {
        ...
        "text": (
            "To sign up, register, or join a program, follow these steps. Step 1: Fill the online application form at organization website. "
...

    {
        ...
        "category": "course",
        "title": "Is my personal data safe? (student privacy FAQ)",
        "text": (
            "Yes — protecting your personal data is a priority. Your information (profile, "
            "documents, payment records) is encrypted both when stored and when sent over the "
            "network, and access is limited by role so only authorized staff can see what they "
            "need to help you. The AI assistant only uses the minimum information needed to answer "
            "your question, and any contact details such as email or phone are automatically "
            "masked before a response is shown. You can ask staff to update or remove your details "
            "at any time, and the organization follows standard data-protection and retention "
            "practices. For specific privacy or data-deletion requests, contact the support team."
        ),
    },
    {
        "id": "trouble-login",
        ...
        "text": (
            "If you cannot log in or sign in (sign-in problems): "
...

    {
        "id": "sys-network",
        "category": "system",
        "title": "Network architecture and firewall rules",
        "text": (
            "The platform uses a three-tier, zero-trust segmented network with a separate "
            "management VLAN. Each tier is its own subnet, and traffic between tiers must pass a "
            "firewall under a default-deny policy. "
            "Tiers: (1) DMZ / Public — 192.168.10.0/24 — holds the load balancer and WAF; only "
            "inbound HTTPS on port 443 is allowed from the internet. (2) Application / API — "
            "192.168.20.0/24 — holds the app server and API gateway; no direct inbound from the "
            "internet, reachable only from the DMZ on 443. (3) Database — 192.168.30.0/24 — holds "
            "the database; it has no internet route at all and accepts connections only from the "
            "app subnet on the database port (5432). (4) Management — 192.168.40.0/24 — holds the "
            "SIEM, jump host, and admin workstations; reachable by VPN only. "
            "Key firewall rules (default-deny baseline): Internet to DMZ WAF on 443 = ALLOW; "
            "Internet to anything else = DENY; DMZ to App subnet on 443 = ALLOW; App subnet to DB "
            "subnet on 5432 = ALLOW; DB subnet outbound to internet = DENY (the database never "
            "calls out); Management to all via VPN on port 22 = ALLOW; any other traffic = DENY. "
            "How the tiers are separated for security: the public can only reach the DMZ load "
            "balancer; the app and database servers are never directly exposed; and even if the app "
            "server were compromised, the database has no internet route, so it cannot be reached "
            "directly from outside. Admin access is out-of-band through the VPN/management subnet."
        ),
    },
    {
        "id": "sys-secure-comms",
        "category": "system",
        "title": "Secure communication — HTTPS, TLS, and API security",
        "text": (
            "All external traffic uses HTTPS with TLS 1.2 or 1.3. The WAF / load balancer "
            "terminates TLS using auto-renewed certificates (e.g. via an ACME certificate "
            "authority) and enforces HSTS so browsers refuse plain HTTP. Internal service-to-service "
            "calls are also encrypted (TLS), and use mutual TLS (mTLS) or signed short-lived tokens "
            "for authentication — defense in depth, so even traffic inside the private network is "
            "protected. "
            "API security: requests carry short-lived JWT / OAuth bearer tokens; the API gateway "
            "validates the token scope and rate-limits per key. Secrets and API keys live in a "
            "secrets manager / vault (e.g. HashiCorp Vault or AWS Secrets Manager), never in code "
            "or config files. "
            "What happens when a user securely connects, step by step: (1) the browser opens an "
            "HTTPS connection and the TLS handshake negotiates keys and verifies the server "
            "certificate; (2) an encrypted channel is established and the user logs in, receiving a "
            "session / JWT token; (3) each subsequent request carries that token over TLS, the WAF "
            "inspects it, and the gateway authenticates and authorizes it against the user's RBAC "
            "role; (4) the app processes the request, queries the database over an encrypted "
            "internal link, and returns the response inside the same TLS tunnel, with the output "
            "guardrail stripping any PII before final delivery."
        ),
    },
    {
        "id": "sys-risks",
        ...
        "text": (
            "Incident response: if a breach or security incident is detected, the response plan "
            "is to (1) contain — isolate affected systems and revoke compromised credentials or "
            "keys; (2) assess — review audit logs to determine scope and impact; (3) eradicate and "
            "recover — patch the root cause and restore from clean backups; (4) notify affected "
            "users and stakeholders as required; and (5) run a post-incident review to close the "
            "gap. "
